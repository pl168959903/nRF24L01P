 ...
 * @retval
 */
static uint8_t ReadPayloadToNodeBuffer( nRF_T* obj, nRF_node_t* node) {
    uint8_t readDataLength;
    if ( node->dynamicPayloadLengthEnabled == true ) { NRF_READ_RX_PAYLOAD_WIDE( obj, &readDataLength ); }
    else {
        readDataLength = node->payloadWide;
    }
    NRF_RX_PAYLOAD( obj, node->buf->data, readDataLength );
    node->buf->length = readDataLength;
...
nRF_statusReg_t nRF_TxPacket( nRF_T* obj, nRF_tx_packet_t* txPacket ) {
    ...
    if ( obj->statusRegister.tx_ds == true ) {
        if ( obj->statusRegister.rx_dr == true ) {
            ReadPayloadToNodeBuffer( obj, txPacket->node);
        }
...
uint8_t nRF_RxPacket( nRF_T* obj ) {
    nRF_node_t *node;
    if(obj->interruptFlag == true){
        node = obj->rxNode[obj->statusRegister.rx_p_no];
    }
    ReadPayloadToNodeBuffer(obj, node);
    return obj->statusRegister.rx_p_no;
}
